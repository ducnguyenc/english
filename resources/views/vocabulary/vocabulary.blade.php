    ...
    <div id="accordion" class="row mr-0 ml-0">
        ...
        <div class="card col text-center">
            <div class="card-header" id="heading{{ $day }}">
                <h5 class="mb-0">
                    <button class="btn btn-link" data-toggle="collapse" data-target="#collapse{{ $day }}" aria-expanded="@if ($day==$days[0]) true @endif" aria-controls="collapse{{ $day }}">
                        Day {{ $day }}
                    </button>
                </h5>
            </div>
        ...
        <div>
            @foreach ($days as $day)
            <div id="collapse{{ $day }}" class="collapse @if ($day==$days[0]) show @endif" aria-labelledby="heading{{ $day }}" data-parent="#accordion">
                <div class="card-body">
                    @if ($day == 1)
                    <form action="{{ route('english.vocabulary.store') }}" method="post">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <input type="text" class="form-control @error('english') is-invalid @enderror" id="english" name="english" value="{{ old('english') }}" placeholder="English">
                                @error('english')
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <input type="text" class="form-control @error('vietnamese') is-invalid @enderror" id="vietnamese" name="vietnamese" value="{{ old('vietnamese') }}" placeholder="Vietnamese">
                                @error('vietnamese')
                                <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <table class="table table-bordered mt-1">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="text" class="form-control check{{ $day }}" placeholder="Check" id="check{{ $day }}id{{ $vocabulary->id }}" data-id="@if ($vocabulary->status) {{ $vocabulary->english }}@else{{ $vocabulary->vietnamese }} @endif">
                                </td>
                                <td>
                                    <button class="btn show{{ $day }}" data-id="{{ $vocabulary->english }},{{ $vocabulary->status }},{{ $vocabulary->id }}">
                                        Show
                                    </button>
                                    <button class="btn btn-danger" type="button" data-toggle="modal" data-target="#exampleModal{{ $vocabulary->id }}">
                                        Delete
                                    </button>
                                    <div class="modal fade" id="exampleModal{{ $vocabulary->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-footer">
                                                    ...
                                                    <button type="button" class="btn btn-danger deleteVocabulary{{ $day }}" data-id="{{ $vocabulary->id }}">Delete
                                                    </button>
                                                </div>
                                            </div>
                                        </div>
        ...
        $.each(day, function(key, value) {
            ...
            $(".check" + value).keyup(function() {
                ...
                $.each(vietnamese, function(key, value) {
                    vietnamese[key] = value.trim().toUpperCase()
                });
                if (jQuery.inArray($(this).val().trim().toUpperCase(), vietnamese) !== -1 || $(this).attr(
